#!/bin/bash
# Скрипт для получения и обновления SSL сертификатов Let's Encrypt
# Использование: 
#   ./ssl_cert.sh [email]           - получить новые сертификаты
#   ./ssl_cert.sh renew              - обновить существующие сертификаты
...
# Функция для получения новых сертификатов
obtain_cert() {
    ...
    echo "   Для ручного обновления выполните: ./ssl_cert.sh renew"
}
...
if [ "$1" = "renew" ]; then
    ...
    echo "ℹ️  Сертификаты уже существуют. Используйте './ssl_cert.sh renew' для обновления."
    echo "   Или './ssl_cert.sh [email]' для получения новых сертификатов."
    exit 0
else
    obtain_cert
fi
