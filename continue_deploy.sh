#!/bin/bash
# Продолжение развертывания после добавления SSH ключа
# Выполните на сервере после добавления SSH ключа в GitHub

set -e

echo "🚀 Продолжение развертывания Panel"
echo ""

# Проверка SSH подключения к GitHub
echo "🔐 Проверка SSH подключения к GitHub..."
ssh -T git@github.com 2>&1 | grep -q "successfully authenticated" && echo "✅ SSH ключ работает" || echo "⚠️  Проверьте, что ключ добавлен в GitHub"
...
if [ -d .git ]; then
    ...
    git clone git@github.com:drads1337/control_panel.git .
    git checkout develop
fi
