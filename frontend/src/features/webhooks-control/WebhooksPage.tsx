              ...
              <CardHeader className="p-3 border-b space-y-3">
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                        ...
                        <CardTitle className="text-sm font-bold uppercase tracking-wide">Endpoints</CardTitle>
                    </div>
                    <Button size="sm" className="h-6 text-[10px] gap-1 px-2.5">
                    ...
                    {WEBHOOKS.map(hook => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5 w-full bg-muted/50 p-1.5 rounded border border-border/50">
                            <Badge variant="outline" className="text-[9px] h-4 px-1 rounded-[3px] bg-background border-border text-muted-foreground font-mono">POST</Badge>
                            <span className="text-[10px] font-mono text-muted-foreground truncate flex-1">{hook.url}</span>
                        </div>

                        <div className="flex flex-wrap gap-1">
                            {hook.events.slice(0, 3).map(evt => (
                                <span key={evt} className="text-[9px] px-1.5 py-0.5 bg-muted rounded-sm border text-muted-foreground font-medium">
                                    {evt}
                                </span>
                            ))}
                            {hook.events.length > 3 && (
                                <span className="text-[9px] px-1.5 py-0.5 text-muted-foreground">+{hook.events.length - 3}</span>
                            )}
                        </div>
                      </button>
                    ))}
                    ...
                    <CardHeader className="p-4 pb-0 flex flex-row items-start justify-between space-y-0">
                        <div className="space-y-1">
                            <div className="flex items-center gap-3">
                                <h2 className="text-lg font-bold">{selectedHook.name}</h2>
                                <Badge variant="outline" className={cn("text-[10px] h-5 px-1.5 uppercase", getStatusColor(selectedHook.status))}>
                                    {selectedHook.status}
                                </Badge>
                            </div>
                    ...
                    <CardContent className="p-4">
                        ...
                        <div className="flex items-center gap-3 p-1.5 pl-3 pr-1.5 rounded-md border bg-muted/20">
                            <div className="flex items-center gap-2 text-muted-foreground shrink-0">
                                ...
                                <span className="text-[10px] font-bold uppercase tracking-wide">Signing Secret</span>
                            </div>
                            ...
                            <Input 
                                ...
                                readOnly
                                className="border-0 bg-transparent h-6 text-xs font-mono focus-visible:ring-0 px-0 shadow-none flex-1 text-foreground"
                            />
                        ...
                        <div className="grid grid-cols-3 gap-4 mt-4">
                            <div className="p-3 rounded-md border bg-card flex flex-col items-center justify-center gap-1 shadow-sm">
                                ...
                                <div className={cn("text-xl font-bold", selectedHook.stats.successRate > 98 ? 'text-emerald-500' : 'text-amber-500')}>
                                    {selectedHook.stats.successRate}%
                                </div>
                            </div>
                            <div className="p-3 rounded-md border bg-card flex flex-col items-center justify-center gap-1 shadow-sm">
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Deliveries</span>
                                <div className="text-xl font-bold">{selectedHook.stats.deliveries.toLocaleString()}</div>
                            </div>
                            <div className="p-3 rounded-md border bg-card flex flex-col items-center justify-center gap-1 shadow-sm">
                                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Avg Latency</span>
                                <div className="text-xl font-bold">{selectedHook.stats.latency}ms</div>
                            </div>
                        </div>
                    </CardContent>
                </Card>
                ...
                <Card className="flex flex-col flex-1 border bg-background shadow-sm overflow-hidden">
                    <CardHeader className="p-3 border-b bg-muted/10 flex flex-row items-center justify-between space-y-0">
                        ...
                        <div className="flex gap-2">
                            <Button variant="outline" size="sm" className="h-6 text-[10px] gap-1 px-2 border-dashed">
                                <Filter className="size-3" /> Filter
                            </Button>
                            <Button variant="ghost" size="sm" className="h-6 text-[10px] gap-1 px-2 text-primary hover:text-primary hover:bg-primary/10">
                    ...
                    <CardContent className="p-0 flex-1 overflow-hidden">
                        <ScrollArea className="h-full">
                            <table className="w-full text-left border-collapse">
                                <thead className="bg-muted/30 sticky top-0 z-10 text-[10px] font-bold uppercase text-muted-foreground tracking-wider">
                                    <tr>
                                    ...
                                    {LOGS.map((log) => (
                                        <tr key={log.id} className="group hover:bg-muted/30 transition-colors cursor-default">
                                            <td className="px-4 py-2.5">
                                                <Badge variant="outline" className={cn("text-[10px] px-1.5 h-5 font-mono font-normal border", getStatusCodeBadge(log.status))}>
                                                    {log.status}
                                                </Badge>
                                            </td>
                                            <td className="px-4 py-2.5">
                                            ...
                                            <td className="px-4 py-2.5 text-right">
                                                <div className="flex flex-col items-end gap-0.5">
                                                    <span className="text-[10px] font-bold font-mono">{log.latency}</span>
