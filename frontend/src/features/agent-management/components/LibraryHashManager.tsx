...
import {
  getAgentLibraryHashes,
  addAgentLibraryHash,
  deleteAgentLibraryHash,
  getAgentLibraryHashSettings,
  updateAgentLibraryHashSettings,
  type LibraryHash,
  type LibraryHashSettings
} from '@/entities/agent/api/agent';
...
export default function LibraryHashManager({ open, onOpenChange, agentId }: LibraryHashManagerProps) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const [hashesResponse, settingsResponse] = await Promise.all([
        getAgentLibraryHashes(agentId),
        getAgentLibraryHashSettings(agentId)
      ]);
  ...
  const handleAddHash = async () => {
    ...
    try {
      setSaving(true);
      await addAgentLibraryHash(agentId, {
        hash_sha256: hash,
  ...
  const handleDeleteHash = async (hashId: number) => {
    ...
    try {
      setSaving(true);
      await deleteAgentLibraryHash(agentId, hashId);
  ...
  const handleSaveSettings = async () => {
    ...
    try {
      setSaving(true);
      await updateAgentLibraryHashSettings(agentId, settings);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Manage SHA-256 hashes of library builds for this agent. Clients must use a matching hash to connect.
          </DialogDescription>
        </DialogHeader>
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-4 p-4 border rounded-lg">
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Enable or disable library hash verification for this agent
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="space-y-0.5">
                  <Label htmlFor="hash-check-enabled-agent">Enable Hash Verification</Label>
                  <p className="text-sm text-muted-foreground">
                    When enabled, only clients with matching library hashes can connect
                  </p>
                </div>
                <Switch
                  id="hash-check-enabled-agent"
            ...
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    List of SHA-256 hashes that are allowed for this agent
                  </p>
                </div>
                <Button
              ...
              {showAddForm && (
                <div className="p-4 border rounded-lg space-y-4 bg-muted/50">
                  <div className="space-y-2">
                    <Label htmlFor="hash-sha256-agent">SHA-256 Hash *</Label>
                    <Input
                      id="hash-sha256-agent"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="hash-version-agent">Version (optional)</Label>
                    <Input
                      id="hash-version-agent"
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="hash-description-agent">Description (optional)</Label>
                    <Textarea
                      id="hash-description-agent"
