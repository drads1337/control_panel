...
const ProductItem = React.memo<{
  ...
  const checkboxId = `${prefix}-product-${product.id}`;
  ...
  return (
    ...
    >
      ...
      <Label 
        htmlFor={checkboxId} 
        className="flex-1 min-w-0 font-medium cursor-pointer text-xs flex items-center gap-1.5 leading-4"
      >
        <span className="truncate">{product.name}</span>
        <Badge variant="outline" className="text-xs h-4 px-1 font-normal shrink-0">
          v{product.version}
        </Badge>
      </Label>
    </div>
...
const SectionHeader: React.FC<{
  ...
  <div className="flex items-start justify-between py-1">
    <div className="flex items-start gap-2">
      ...
      <Label htmlFor={checkboxId} className="text-sm font-medium cursor-pointer leading-5 pt-0.5">
        {title} ({count})
      </Label>
    </div>
    {selectedCount > 0 && (
