                ...
                <>
                  {user.expires_at ? (
                    <span className="text-xs text-muted-foreground">
                      • Expires{" "}
                      {new Date(user.expires_at).toLocaleDateString("en-US", {
                        month: "short",
                        day: "numeric",
                        year: "numeric",
                      })}
...
const EmployeesTab: React.FC<EmployeesTabProps> = ({
  ...
  const [selectedUserIdForTopup, setSelectedUserIdForTopup] =
    useState<number | null>(null)
  const [selectedUserNameForTopup, setSelectedUserNameForTopup] =
    useState<string>("")
  const [selectedUserBalanceForTopup, setSelectedUserBalanceForTopup] =
    useState<number>(0)
