...
class MasterKeyManager:
    ...
    def encrypt_with_master_key_legacy(data: str, master_key: str) -> str:
        ...
        try:
            ...
            # SECURITY: Never log sensitive data or full keys
    ...
    def decrypt_with_master_key_legacy(encrypted_data: str, master_key: str) -> str:
        ...
        try:
            ...
            # Try to decode base64 - handle both regular and URL-safe base64
            combined = None
            decode_errors = []
            try:
                # First try regular base64
                ...
                try:
                    ...
                    # If both fail, try adding padding and retry
                    try:
                        ...
                        # Last attempt: try URL-safe with padding
                        try:
                            missing_padding = len(encrypted_data) % 4
                            padded_data = (
                                encrypted_data + ("=" * (4 - missing_padding))
            ...
            # Log at debug level since decryption failures are expected during key fallback attempts
...
class ProjectCrypto:
    ...
    def generate_project_key() -> str:
        ...
        key = SecureCrypto.generate_key()  # Returns 32 bytes
        # Return as hex string (64 characters) for consistency with master keys
        return key.hex()

    @staticmethod
    def encrypt_for_project(data: str, project_key: str) -> str:
        """Encrypt data for a specific project using AES-256-GCM."""
        # Project key can be hex string (64 chars) or base64 encoded
        try:
            # Try hex first (preferred format)
...
# Project-specific encryption functions that work with project IDs
def encrypt_data_with_project_key(data: dict, project_id: int, use_gcm: bool = True) -> str:
    ...
    # First, try AES Key from ProjectEncryptionKeys (this is what clients use)
    try:
    ...
    try:
        ...
        if encryption_keys and encryption_keys.aes_key:
            ...
            try:
                ...
                # Try the new standard method as fallback
                logging.debug(
