...
@require_project_with_grace_period
@require_project_isolation
def search_keys(current_user, project_id=None):
    """
    Search keys using POST to prevent PII leakage in URL query parameters.
    ...
    my_keys = data.get("my_keys", False)
    status_arg = data.get("status")
    status_filter = None if (status_arg is None or status_arg.lower() == "all") else status_arg
    ...
    logger.info(
        f"🔑 POST /api/keys/search - user_id={current_user.id}, project_id={current_user.project_id}, "
        f"filters={filters}"
    )
    ...
    logger.info(
        f"✅ POST /api/keys/search response - keys_count={len(keys)}, total_count={total_count}, "
        f"page={filters['page']}, per_page={filters['per_page']}"
    )
