...
def upgrade():
    ...
    # User table - search by username, first_name, last_name
    op.execute("""
        ALTER TABLE "user" 
        ADD COLUMN IF NOT EXISTS search_vector tsvector;
    """)
    ...
    # Server table - search by name, ip_address, description
    op.execute("""
        ALTER TABLE server 
        ADD COLUMN IF NOT EXISTS search_vector tsvector;
    """)
    ...
    # UserActivity table - search by action, ip_address, country, city, details, user_agent
    op.execute("""
    ...
    # ChangelogEntry table - search by version, title, description, changes
    op.execute("""
