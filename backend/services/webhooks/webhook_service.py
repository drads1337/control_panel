...
class WebhookService:
    ...
    def create_webhook(
        ...
        validation_service = self._webhook_validation_service
    ...
    def update_webhook(self, webhook_id: int, project_id: Optional[int] = None, **kwargs) -> Dict:
        ...
        if "events" in kwargs:
            if not self._webhook_formatting_service:
                raise ServiceError(
                    "Webhook Formatting dependency not injected",
                    status_code=500
                )
            formatting_service = self._webhook_formatting_service
    ...
    def trigger_webhook(self, event: str, data: Dict, project_id: Optional[int] = None) -> bool:
        ...
        if not self._webhook_execution_service:
            raise ServiceError(
                "Webhook Execution dependency not injected",
                status_code=500
            )
    ...
    def get_webhook_logs(self, webhook_id: int, limit: int = 100) -> List[Dict]:
        """Get webhook logs"""
        if not self._webhook_logging_service:
            raise ServiceError(
                "Webhook Logging dependency not injected",
                status_code=500
            )
    ...
    def test_webhook(self, webhook_id: int) -> Dict:
        ...
        if not self._webhook_testing_service:
            raise ServiceError(
                "Webhook Testing dependency not injected",
                status_code=500
            )
    ...
    def process_pending_webhook_tasks(self, batch_size: int = 50) -> Dict[str, int]:
        ...
        if not self._webhook_pending_task_service:
            raise ServiceError(
                "Webhook Pending Task dependency not injected",
                status_code=500
            )
    ...
    def _generate_secret(self) -> str:
        ...
        if not self._webhook_crypto_service:
            raise ServiceError(
                "Webhook Crypto dependency not injected",
                status_code=500
            )
        crypto_service = self._webhook_crypto_service
