...
class LibraryHashService:
    ...
    def validate_library_hash(
        self, key_obj: Key, hash_sha256: str
        ...
        Args:
            ...
            hash_sha256: SHA-256 хэш библиотеки от клиента (в hex формате)
        ...
        """
        if not hash_sha256:
            # Если хэш не передан - пропустить проверку (для обратной совместимости)
            return True, "", None

        # Нормализовать хэш (lowercase, убрать пробелы)
        ...
        if len(hash_sha256) != 64 or not all(c in "0123456789abcdef" for c in hash_sha256):
            logger.warning(
                f"INVALID_LIBRARY_HASH_FORMAT hash_length={len(hash_sha256)} "
                f"key_id={key_obj.id if key_obj else None}"
            )
            return False, "Invalid library hash format", None

        # Проверить для Agent (если используется loader)
        if key_obj and key_obj.agent_id:
            is_valid, error_msg = self._validate_agent_hash(key_obj.agent_id, hash_sha256)
            if not is_valid:
        ...
        if key_obj and key_obj.product_id:
            is_valid, error_msg = self._validate_product_hash(
                key_obj.product_id, hash_sha256
            )
            if not is_valid:
    ...
    def _validate_agent_hash(
        self, agent_id: int, hash_sha256: str
    ) -> Tuple[bool, str]:
        """
    ...
    def _validate_product_hash(
        self, product_id: int, hash_sha256: str
    ) -> Tuple[bool, str]:
        """
