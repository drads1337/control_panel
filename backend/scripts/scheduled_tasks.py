...
class ScheduledTaskProcessor:
    ...
    def ensure_user_activity_partitions(self):
        """Ensure user_activity partitions exist for next months"""
        try:
            self.logger.info("Ensuring user_activity partitions exist")
            ...
            try:
                ...
                session.commit()
                self.logger.info("User activity partitions ensured successfully")
            except Exception as e:
                self.logger.error(f"Failed to ensure user_activity partitions: {e}")
                session.rollback()
            finally:
                session.close()
