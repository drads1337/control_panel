...
def check_tables_exist():
    ...
    required_tables = ['user', 'project', 'role', 'user_role']
...
def create_owner():
    ...
    with app.app_context():
        try:
            ...
            if not tables_exist:
                print("❌ Ошибка: Таблицы базы данных не существуют.")
                print(f"   Отсутствующие таблицы: {', '.join(missing_tables)}")
                print("\n💡 Решение: Сначала необходимо создать таблицы, выполнив миграции.")
                print("   Выполните одну из следующих команд:")
                print("   1. python backend/scripts/apply_migration.py")
                print("   2. flask db upgrade")
                print("   3. python backend/scripts/recreate_database.py (если база данных пустая)")
                return
            ...
            try:
                ...
                # Если таблица существует, но запрос все равно не работает
                if 'does not exist' in str(query_error).lower() or 'relation' in str(query_error).lower():
                    print("❌ Ошибка: Таблицы базы данных не существуют или не доступны.")
                    print(f"   Ошибка: {str(query_error)}")
                    print("\n💡 Решение: Сначала необходимо создать таблицы, выполнив миграции.")
                    print("   Выполните одну из следующих команд:")
                    print("   1. python backend/scripts/apply_migration.py")
                    print("   2. flask db upgrade")
