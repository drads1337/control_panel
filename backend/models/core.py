...
class Project(db.Model):
    ...
    unique_id = db.Column(db.String(10), unique=True, nullable=False)  # 10-digit unique identifier
    name = db.Column(db.String(128), unique=True, nullable=False)
    description = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    # Can add owner_id if needed to bind to owner
    logo_url = db.Column(db.String(256), nullable=True)
    status = db.Column(db.String(32), default="active")  # active, inactive, expired
    # Project admin is now handled through ProjectAdmin model to avoid circular dependencies
    # Legacy field for backward compatibility - will be deprecated
    admin_id = db.Column(db.Integer, nullable=True)  # DEPRECATED: Use ProjectAdmin model instead
    ...
    # Field to store original subscription data when project is set to inactive
    ...
    # Storage limit field (in bytes, default 3 GB for APK files)
    storage_limit = db.Column(db.BigInteger, default=3221225472)  # 3 GB in bytes

    # Denormalized statistics for performance optimization
    # These fields are updated automatically when resources are created/deleted/updated
    # This avoids expensive JOIN queries with subqueries in get_projects_cached
    ...
    def subscription_status_display(self):
        ...
        # If project is inactive, show the actual subscription status
        ...
        # For active projects, check if subscription is valid
        if not self.is_active:
            return "expired"

        # If project is active and subscription is valid, check expiry status
        if self.subscription_expires_at:
            days_left = self.days_until_expiry
            if days_left <= 0:
                return "expired"
        ...
        # If no subscription_expires_at, use the subscription_status field
        if self.subscription_status:
            return self.subscription_status

        # Only return 'trial' as a last resort
        return "trial"
...
class ProjectEncryptionKeys(db.Model):
    ...
    # RSA public key certificate (PEM format)
    public_key_cert = db.Column(db.Text, nullable=False)

    # RSA private key (encrypted with project-specific password, PEM format)
    private_key_encrypted = db.Column(db.Text, nullable=False)

    # Key generation timestamp
    created_at = db.Column(db.DateTime, default=datetime.utcnow)

    # Key metadata (algorithm, key size, etc.)
    key_metadata = db.Column(db.Text, nullable=True)  # JSON string
...
class ProjectSettings(db.Model):
    ...
    # Security settings - Constants as per requirements
    ...
    max_sessions_per_user = db.Column(db.Integer, default=5)  # Constant: 5 sessions
    log_retention_days = db.Column(db.Integer, default=60)  # Constant: 60 days
    ...
    # Security features - Enabled by default as per requirements
    two_factor_auth_required = db.Column(db.Boolean, default=False)  # Disabled by default
    password_complexity_required = db.Column(db.Boolean, default=True)
    session_fingerprinting = db.Column(db.Boolean, default=True)  # Track and validate fingerprints
    ip_whitelist_enabled = db.Column(db.Boolean, default=False)
    ip_whitelist = db.Column(db.Text, nullable=True)  # JSON array of IP addresses
    rate_limiting_enabled = db.Column(db.Boolean, default=True)  # Constant: enabled
    rate_limit_requests_per_minute = db.Column(
        db.Integer, default=60
    )  # Constant: 60 requests/minute
    vpn_blocking_enabled = db.Column(db.Boolean, default=False)
    security_logging_enabled = db.Column(db.Boolean, default=True)  # Record all security events
    suspicious_activity_check_enabled = db.Column(
        db.Boolean, default=True
    )  # Analyze behavior patterns
    session_limiting_enabled = db.Column(db.Boolean, default=True)  # Limit concurrent sessions
    auto_log_cleanup_enabled = db.Column(db.Boolean, default=True)  # Auto delete old logs
    ...
    appearance_settings = db.Column(db.Text, nullable=True)  # JSON string

    # Project-specific master key for encryption/decryption
    project_master_key = db.Column(db.Text, nullable=True)  # 64-character hex string

    # Registration controls
    invite_code_required = db.Column(
        db.Boolean, default=True
    )  # Require invite code for user self-registration
    ...
    offline_auth_enabled = db.Column(db.Boolean, default=False)  # Enable offline authentication with cached tickets
    offline_ticket_expiration_hours = db.Column(db.Integer, default=12)  # Offline ticket expiration time in hours (default: 12 hours)
...
class User(db.Model):
    ...
    # Denormalized key statistics for performance optimization
    # These fields are updated automatically when keys are created/deleted/updated
    total_keys = db.Column(db.Integer, default=0, nullable=False)
    active_keys = db.Column(db.Integer, default=0, nullable=False)
...
class UserActivity(db.Model):
    ...
    user_agent = db.Column(db.String(512), nullable=True)  # Added user_agent field
    country = db.Column(db.String(64), nullable=True)
    city = db.Column(db.String(64), nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    project_id = db.Column(db.Integer, db.ForeignKey("project.id"), nullable=True)
    project = db.relationship("Project", backref="user_activities")
    details = db.Column(db.Text, nullable=True)  # New field for details
    session_id = db.Column(db.String(128), nullable=True)  # Session ID for grouping
...
class UserGamePermission(db.Model):
    ...
    )  # Access denied by default - must be explicitly granted
    project_id = db.Column(
        db.Integer, db.ForeignKey("project.id"), nullable=True
    )  # Added project_id field
    granted_at = db.Column(db.DateTime, default=datetime.utcnow)  # When permission was granted
    granted_by = db.Column(
        db.String(50), default="admin"
    )  # Who granted the permission (admin, access_code, etc.)
    access_code = db.Column(
        db.String(255), nullable=True
    )  # Access code if permission was obtained through code
...
class SystemSettings(db.Model):
    ...
    setting_type = db.Column(db.String(32), default="string")  # string, integer, boolean, json
    category = db.Column(
        db.String(64), nullable=False
    )  # general, security, projects, notifications, api
    description = db.Column(db.Text, nullable=True)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    updated_at = db.Column(db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow)
...
class SystemBackup(db.Model):
    ...
    backup_type = db.Column(db.String(32), default="full")  # full, incremental
    status = db.Column(db.String(32), default="completed")  # completed, failed, in_progress
    created_by = db.Column(db.Integer, db.ForeignKey("user.id"), nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
...
class ProjectInviteCode(db.Model):
    ...
    expires_at = db.Column(db.DateTime, nullable=True)  # Code expiration date
    project_id = db.Column(
        db.Integer, db.ForeignKey("project.id"), nullable=True
    )  # Added project_id field
    ...
    auto_delete_at = db.Column(db.DateTime, nullable=True)  # When to auto-delete if unused
