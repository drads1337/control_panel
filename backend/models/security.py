...
class TwoFactorAuth(db.Model):
    ...
    secret_key = db.Column(db.String(32), nullable=True)  # Base32 encoded secret for TOTP
...
class TwoFactorSession(db.Model):
    ...
    )  # Temporary token after password verification
...
class TwoFactorBackupCode(db.Model):
    ...
    # Code data
    code_hash = db.Column(db.String(64), nullable=False)  # Hashed backup code
...
class BlockedFingerprint(db.Model):
    ...
    )  # manual, automatic, behavioral, geo, rate_limit
    severity = db.Column(db.String(16), default="medium")  # low, medium, high, critical
    threat_score = db.Column(db.Integer, default=0)  # 0-100 threat assessment score
    ...
    associated_keys = db.Column(db.Text, nullable=True)  # JSON array of associated user keys

    # Auto-unblock settings
    auto_unblock_enabled = db.Column(db.Boolean, default=False)
    unblock_conditions = db.Column(db.Text, nullable=True)  # JSON conditions for auto-unblock
    ...
    # Composite unique constraint: same fingerprint can be blocked for different projects
    __table_args__ = (
        db.UniqueConstraint(
...
class SecurityRule(db.Model):
    ...
    )  # fingerprint_block, ip_block, rate_limit, geo_block, behavioral

    # Rule conditions (JSON)
    conditions = db.Column(db.Text, nullable=False)  # JSON conditions for rule activation

    # Rule actions
    action_type = db.Column(db.String(32), nullable=False)  # block, warn, log, notify
    action_params = db.Column(db.Text, nullable=True)  # JSON parameters for action

    # Rule settings
    is_active = db.Column(db.Boolean, default=True)
    priority = db.Column(db.Integer, default=100)  # Higher number = higher priority
    cooldown_minutes = db.Column(db.Integer, default=60)  # Cooldown between rule executions
...
class BlockedIP(db.Model):
    ...
    )  # general, spam, abuse, fraud, malware, suspicious, violation
    severity = db.Column(db.String(16), default="medium")  # low, medium, high, critical
    threat_score = db.Column(db.Integer, default=0)  # 0-100 threat assessment score
    country = db.Column(db.String(64), nullable=True)
    city = db.Column(db.String(64), nullable=True)
    ...
    # Composite unique constraint: same IP can be blocked for different projects
    __table_args__ = (
...
class BlockedHWID(db.Model):
    ...
    first_seen = db.Column(db.DateTime, default=datetime.utcnow)
    last_seen = db.Column(db.DateTime, default=datetime.utcnow)
    associated_ips = db.Column(db.Text, nullable=True)  # JSON array of associated IP addresses
    ...
    # Composite unique constraint: same HWID can be blocked for different projects
    __table_args__ = (
...
class SecurityEvent(db.Model):
    ...
    )  # fingerprint_block, suspicious_activity, etc.
    severity = db.Column(db.String(16), default="medium")  # low, medium, high, critical
...
class SecurityAnalytics(db.Model):
    ...
    metric_type = db.Column(
        db.String(64), nullable=False
    )  # blocked_fingerprints, suspicious_ips, etc.
