    ...
    with celery_db_session() as session:
        try:
            ...
            for i in range(count):
                try:
                    if task_service and task_id and (i + 1) % max(1, total // 10) == 0:
            ...
            logger.error(f"Unexpected error in bulk_create_keys_task: {e}", exc_info=True)
...
def bulk_create_loader_keys_task(
    ...
    with celery_db_session() as session:
        try:
            ...
            for i in range(count):
                try:
                    if task_service and task_id and (i + 1) % max(1, count // 10) == 0:
            ...
            if created_keys:
                ...
                error_msg = "No keys were created"
                if task_service and task_id:
                    task_service.update_task_status(task_id, "failed", error=error_msg)
                return {"status": "error", "error": error_msg}

        except Exception as e:
            logger.error(f"Unexpected error in bulk_create_loader_keys_task: {e}", exc_info=True)
